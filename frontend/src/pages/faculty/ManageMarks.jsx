...
function ManageMarks() {
    ...
    useEffect(() => {
        ...
        // Fetch all assigned students for this faculty
        if (facultyId) {
            api.get(`/faculties/${facultyId}/students`).then(res => setAssignedStudents(res.data)).catch(console.error);
        }
    }, [facultyId]);

    // Fetch existing exam marks when Exam tab subject changes
    useEffect(() => {
        if (activeTab === 'exam' && examSubject) {
            api.get(`/marks/subject/${examSubject}`)
                .then(res => {
                    const prepData = {};
                    res.data.forEach(m => {
                        prepData[m.student_id] = m.internal_mark !== null ? m.internal_mark : '';
                    });
                    setExamMarksData(prepData);
                })
                .catch(err => {
                    console.error('Failed to load existing marks', err);
                });
        }
    }, [activeTab, examSubject]);

    // Fetch submitted assignments when Assign tab subject changes
    useEffect(() => {
        if (activeTab === 'assignment' && assignSubject) {
            api.get(`/assignments/submitted/${assignSubject}`)
               .then(res => {
                   // We only care about students assigned to THIS faculty who submitted
                   const facultyStudentIds = new Set(assignedStudents.map(s => s.id));
                   const filtered = res.data.filter(sub => facultyStudentIds.has(sub.student_id));
                   setSubmittedAssignments(filtered);
               ...
               .catch(err => toast.error('Failed to load submitted assignments'));
        }
    }, [activeTab, assignSubject, assignedStudents]);
    ...
    const handleAssignSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await api.post('/assignments/bulk', { assignments: payload });
